            ...
            <>
              <ResponsiveContainer width="100%" height={140}>
                <BarChart data={data?.paymentChart ?? []} barSize={32}>
                  ...
                  <Tooltip />
                  <Bar dataKey="value" radius={[4, 4, 0, 0]} name="Orders" fill="#6366f1" />
                </BarChart>
              </ResponsiveContainer>
          ...
          <div className="divide-y divide-border">
            {(data?.recentOrders ?? []).length === 0 && (
              <div className="py-10 text-center text-sm text-muted-foreground">
                No orders yet — <Link to="/pos" className="text-primary hover:underline font-medium">make your first sale</Link>
              </div>
            )}
          ...
          <div className="space-y-3">
            {(data?.lowStock ?? []).length === 0 ? (
              ...
              <>
                ...
                <Link to="/products">
                  <Button variant="outline" size="sm" className="w-full mt-2 text-xs">Manage Inventory</Button>
                </Link>
              </>
            )}
          </div>
